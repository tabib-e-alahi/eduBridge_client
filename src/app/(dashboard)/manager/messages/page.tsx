...
import { MessageSquare, Search, Send, Paperclip, Phone, Video, MoreVertical } from "lucide-react";
...
export default function ManagerMessagesPage() {
  ...
  const { data: convoData, isLoading, isError, refetch } = useConversations();
  ...
  useEffect(() => {
    if (contacts.length > 0 && !activeContactId) setActiveContactId(contacts[0].id);
  ...
  return (
    <div className="h-[calc(100vh-80px-64px)] flex flex-col animate-in fade-in duration-500">
      <PageHeader title="Messages" subtitle="Communicate with your students and support." />
          ...
          {!activeContact ? (
            <div className="flex-1 flex items-center justify-center text-center p-8">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Choose a student to start messaging.</p>
              </div>
            </div>
          ) : (
            <>
              <div className="p-4 border-b flex justify-between items-center bg-muted/5">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" className="h-9 w-9 rounded-[0.5rem] text-muted-foreground"><Phone className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" className="h-9 w-9 rounded-[0.5rem] text-muted-foreground"><Video className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" className="h-9 w-9 rounded-[0.5rem] text-muted-foreground"><MoreVertical className="h-4 w-4" /></Button>
                </div>
              </div>
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex flex-col gap-2 max-w-[80%] ${isMine ? "items-end self-end ml-auto" : "items-start"}`}>
                        <div className={`p-3 rounded-[0.625rem] text-sm font-medium ${isMine ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-muted rounded-tl-none"}`}>
                          {msg.content}
                        </div>
              ...
              <div className="p-4 border-t bg-muted/5">
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter" && messageInput.trim()) handleSend(); }}
                  />
                  <Button className="h-12 w-12 rounded-[0.625rem] shrink-0 font-bold" onClick={handleSend} disabled={!messageInput.trim() || sendMessage.isPending}>
